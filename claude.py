...
def find_optimal_build_location(width, length, margin=5):
    """
    Finds the optimal location to build a house by:
    1. Avoiding water
    2. Finding the flattest area possible
    3. Ensuring the build area is large enough
    
    Returns (x, z, y) coordinates and the flatness score for the best location
    """
    print("Finding optimal build location...")
    
    # Get necessary heightmaps
    ...
    # Calculate the difference between heightmaps to identify water
    water_diff = terrain_hmap - water_hmap
    
    # Calculate dimensions of the area we need (including margins)
    req_width = width + (2 * margin)
    req_length = length + (2 * margin)
    
    # Maximum coordinates we can check (accounting for required space)
    max_x = terrain_hmap.shape[0] - req_width
    max_z = terrain_hmap.shape[1] - req_length
    
    if max_x <= 0 or max_z <= 0:
        print("Warning: Building is too large for the available area!")
        # Fall back to center if area is too small
        center_x = terrain_hmap.shape[0] // 2
        center_z = terrain_hmap.shape[1] // 2
        avg_height = int(np.mean(terrain_hmap[
            max(0, center_x - width//2):min(terrain_hmap.shape[0], center_x + width//2),
            max(0, center_z - length//2):min(terrain_hmap.shape[1], center_z + length//2)
        ]))
        return STARTX + center_x, STARTZ + center_z, avg_height, 999
    
    best_x, best_z = 0, 0
    best_flatness = float('inf')
    best_y = 0
    
    # Scan the area in grid steps to reduce computation
    step_size = max(1, min(max_x, max_z) // 20)
    
    for x in range(0, max_x, step_size):
        for z in range(0, max_z, step_size):
            # Extract the region of interest
            region = terrain_hmap[x:x+req_width, z:z+req_length]
            water_region = water_diff[x:x+req_width, z:z+req_length]
            
            # Skip if there's water in the region
            if np.any(water_region > 0):
                continue
                
            # Calculate flatness (standard deviation of heights - lower is better)
            flatness = np.std(region)
            
            # Check if this is flatter than our current best
            if flatness < best_flatness:
                best_flatness = flatness
                best_x = x + (req_width // 2)  # Center of the region
                best_z = z + (req_length // 2)
                best_y = int(np.median(region))  # Use median height for better results
                
                # If we found a perfectly flat spot, no need to continue
                if flatness < 0.01:
                    break
                    
        # Also break outer loop for perfect flatness
        if best_flatness < 0.01:
            break
    
    # If we didn't find a good spot without water, find the flattest spot
    # regardless of water (we'll build a platform)
    if best_flatness == float('inf'):
        print("No perfect dry land found, searching for flattest area...")
        for x in range(0, max_x, step_size):
            for z in range(0, max_z, step_size):
                region = terrain_hmap[x:x+req_width, z:z+req_length]
                flatness = np.std(region)
                ...
                if flatness < best_flatness:
                    best_flatness = flatness
                    best_x = x + (req_width // 2)
                    best_z = z + (req_length // 2)
                    best_y = int(np.median(region))
    ...
    # Fine-tune the search around the best location
    if best_flatness > 0.5 and max_x > req_width and max_z > req_length:
        search_radius = min(10, step_size)
        fine_x_min = max(0, best_x - search_radius)
        fine_x_max = min(terrain_hmap.shape[0] - req_width, best_x + search_radius)
        fine_z_min = max(0, best_z - search_radius)
        fine_z_max = min(terrain_hmap.shape[1] - req_length, best_z + search_radius)
        
        for x in range(fine_x_min, fine_x_max + 1):
            for z in range(fine_z_min, fine_z_max + 1):
                # Skip if out of bounds
                if x + req_width > terrain_hmap.shape[0] or z + req_length > terrain_hmap.shape[1]:
                    continue
                    
                region = terrain_hmap[x:x+req_width, z:z+req_length]
                water_region = water_diff[x:x+req_width, z:z+req_length]
                
                # Prefer areas without water
                if np.any(water_region > 0):
                    continue
    ...
    # Convert to global coordinates and set building platform height
    global_x = STARTX + best_x
    global_z = STARTZ + best_z
    
    # Get blocks at the selected location to check for water and other problematic blocks
    center_x_local = best_x
    center_z_local = best_z
    center_block = None
    
    try:
        center_height = terrain_hmap[center_x_local, center_z_local]
        center_block = WORLDSLICE.getBlock((center_x_local, center_height - 1, center_z_local))
    except (IndexError, KeyError):
        print("Warning: Could not check center block at selected location")
    
    # If center block is water, adjust y upward to build above water
    if center_block and ("water" in center_block or "ice" in center_block):
        print(f"Water detected at build location: {center_block}")
        best_y += 3  # Build 3 blocks above water
    
    print(f"Found optimal build location at: ({global_x}, {best_y}, {global_z}) with flatness: {best_flatness:.2f}")
    return global_x, global_z, best_y, best_flatness
...
def is_water_block(x, z, heights_diff):
    """Check if the given location is water by comparing heightmaps"""
    local_x, local_z = x - STARTX, z - STARTZ
    if 0 <= local_x < heights_diff.shape[0] and 0 <= local_z < heights_diff.shape[1]:
        return heights_diff[local_x, local_z] > 0
    return False
...
def build_luxury_mansion(enhance=True):
    """Build a luxury mansion with advanced terrain adaptation"""
    ...
    # Find the best location to build based on terrain
    xaxis, zaxis, y, flatness = find_optimal_build_location(width, length, margin=10)
    
    print(f"Building at coordinates: ({xaxis}, {y}, {zaxis}) with flatness: {flatness:.2f}")
    
    # Get heightmap data for terrain adaptation
    ...
    if enhance:
        # Only add basement if we're not on water (flatness score helps identify this)
        if flatness < 5.0:  # Arbitrary threshold - lower numbers mean flatter land
...
def main():
    try:
        ...
        ED.flush()  # Make sure all changes are applied
    except KeyboardInterrupt:
        print("Build canceled!")
    except Exception as e:
        print(f"An error occurred: {e}")
    finally:
        # Make sure changes are flushed to the server
        ED.flush()
