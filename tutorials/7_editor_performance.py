...
t1 = time.time()
for point in Box(addY(buildRect.center) + ivec3(-10, 100, -10), (1,1,1)).inner:
...
# Time the placement of 512 blocks.
t1 = time.time()
for point in Box(addY(buildRect.center) + ivec3(-10, 100, 3), (1,1,1)).inner:
    editor.placeBlock(point, Block("stone"))
...
t1 = time.time()
editor.loadWorldSlice(buildRect.centeredSubRect((1,1)), cache=True)
t2 = time.time()
...
t1 = time.time()
for point in buildRect.centeredSubRect((1,1)).toBox(100, 8).inner:
    editor.getBlock(point)
t2 = time.time()
